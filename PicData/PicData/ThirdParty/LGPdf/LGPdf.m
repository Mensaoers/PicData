...
- (void)readyToWrite:(NSString *)passowrd {
    CGRect bounds = CGRectMake(0,0,LGPdf_pageInfo.width,LGPdf_pageInfo.height);

    NSDictionary *info = nil;
    if (passowrd != nil && passowrd.length > 0) {
        info = [NSDictionary dictionaryWithObjectsAndKeys:passowrd, kCGPDFContextOwnerPassword,
                passowrd, kCGPDFContextUserPassword,
                [NSNumber numberWithInt:128], kCGPDFContextEncryptionKeyLength, nil];
    }
    UIGraphicsBeginPDFContextToFile(pdfFileFullPath, bounds, info);
...
- (void)addImage:(LGPdfImage *)element {
    [imageController add:element withContext:LGPdf_pdfContext withNowHeight:0];
