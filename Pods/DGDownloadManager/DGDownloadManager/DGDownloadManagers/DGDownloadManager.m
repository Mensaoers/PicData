...
- (NSString *)getCache:(NSString *)downloadUrl withResponse:(NSURLResponse *)response customCahceName:(NSString *)customCacheName{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *cachePath = self.cachePath.length > 0 ? self.cachePath : [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"DGMusic"];
    // [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"DGMusic"];
