...
2. 支持图片预览功能, 因三方库(YBImageBrowser)限制, 不支持mac包, 现只支持iOS设备(真机/模拟器)
...
1. 点击切换到设置标签进行下载目录的设置, 默认是app的`Documents/PicDownload`目录
...
* 该平台分wap资源和web端区分处理, 本项目浏览的时候访问wap端, 下载的时候使用原图地址, 高清大图
* web地址只有中间图片, 其他的显示不全, 直接访问体验不如手机端呢
...
* 详情页面推荐栏显示不出图片(极端偶现)
* 数据库频繁操作导致的问题, 还是没法解决
* 目前有一个已知bug: 详情页面选择下方的推荐查看, 然后返回上一页下载, 还是下载的下一个图集, 该问题涉及到数据库框架, 等有空换个框架再改, 改动有点耗时
